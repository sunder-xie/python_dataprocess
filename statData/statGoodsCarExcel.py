...
class GoodsCarExcel:
    ...
    def init_liyang(self):
        liyang_sql = "select leyel_id,factory_name ,	car_brand ,	car_series ,	vehicle_type ,	model_year ," \
                     "displacement ,intake_style ,	max_power ,	fuel_type  from db_car_info_all"
    ...
    def read_excel(self, file_address):
        ...
        for rownum in range(1, n_rows):
            ...
            if goods_format not in self.goods_dict.keys():
                self.goods_dict[goods_format] = goods_size
            if goods_format in self.goods_car_dict.keys():
                car_id_set = set(self.goods_car_dict[goods_format])
            else:
                car_id_set = set()
            ...
            for liyang_data in liyang_data_list:
                ...
                if is_ok_liyang:
                    ...
                    liyang_goods_set.add(goods_format)
                ...
                self.goods_car_dict[goods_format] = car_id_set

            # 没有对应到
            if not is_find:
                wrong_str = 'not liyang car.goods_format:{},brand:{}, factory:{}, series:{},model:{},year:{},display:{},intake:{},power:{},燃料类型:{}'.format(
                    goods_format, car_brand, car_factory, car_series, car_model, car_year, car_display, car_intake,
                    car_power, car_flue)
                print wrong_str
                self.wrong_data_list.append(wrong_str)
    ...
    def write_sku_excel(self):
        ...
        insert_list = (u'机油滤清器', u'滤清器尺寸', u'car_id', u'品牌', u'车系', u'车型', u'排量', u'年款', u'名称')
        self.write_excel_cell(the_sheet, 1, insert_list)

        is_sheet_num = 2
        for goods_format, car_id_set in self.goods_car_dict.iteritems():
            goods_size = self.goods_dict[goods_format]
            if len(car_id_set) == 0:
                self.write_excel_cell(the_sheet, is_sheet_num, (
                    goods_format, goods_size, u'None', u'None', u'None', u'None', u'None', u'None', u'None'))
                is_sheet_num += 1
            else:
                for car_id in car_id_set:
                    ...
                    self.write_excel_cell(the_sheet, is_sheet_num, (
                        goods_format, goods_size, str(car_id), str(goods_car_data['brand']),
    ...
    def write_1_n_excel(self):
        ...
        ew = ExcelWriter(workbook=wb)
        the_sheet = wb.create_sheet(u"sku不适配车型", 0)
        insert_list = (u'car_id', u'品牌', u'车系', u'车型', u'排量', u'年款', u'名称', u'机油滤清器', u'滤清器尺寸')
        self.write_excel_cell(the_sheet, 1, insert_list)
        is_sheet_num = 2
    ...
    def write_1_n_liyang_excel(self):
        ...
        ew = ExcelWriter(workbook=wb)
        the_sheet = wb.create_sheet(u"sku不适配车型", 0)
        insert_list = (u'liyang_id', u'品牌', u'厂家', u'车系', u'车型', u'年款', u'排量', u'进气形式', u'最大功率', u'燃料类型', u'机油滤清器', u'滤清器尺寸')
        self.write_excel_cell(the_sheet, 1, insert_list)
        is_sheet_num = 2
        for liyang_id, goods_set in self.liyang_car_goods_dict.iteritems():
            ...
            for goods_format in goods_set:
                ...
                self.write_excel_cell(the_sheet, is_sheet_num, (
                    ...
                    liyang_data['vehicle_type'], liyang_data['model_year'], liyang_data['displacement'],
                    liyang_data['intake_style'],
                    liyang_data['max_power'], liyang_data['fuel_type'], goods_format, goods_size))
                is_sheet_num += 1
...
console_excel = r'/Users/zxg/Documents/work/淘气档口/work/0912机滤匹配/机滤正确数据总表.xlsx'
# console_excel = r'/Users/zxg/Documents/work/淘气档口/work/0912机滤匹配/test.xlsx'
